<<<<<<< HEAD:code for follow up analyses/t_testregression.R
library(tidyverse)
library(naniar)
...
Training<-as.data.frame(cbind(OSQ_daten$DS13_05,
                              OSQ_daten$DS13_06,
                              OSQ_daten$DS13_07,
                              OSQ_daten$DS13_08,
                              OSQ_daten$PR07_04,                 
...
Training$TrainingTotal <- rowSums(subset(Training, select = c(1:8)))              # summing variables to get Total Score for this factor                       
Training$TrainingTotal <- (Training$TrainingTotal/8)                    
#Factor2 [fear of being transparent](DS13_09, DS13_10, DS13_11, DS13_12, PR07_08, PR07_09)

Fear<- as.data.frame(cbind(OSQ_daten$DS13_09,
                           OSQ_daten$DS13_10,
                           OSQ_daten$DS13_11,                   
                           OSQ_daten$DS13_12,
                           OSQ_daten$PR07_08,
...
Fear$FearTotal <- rowSums(subset(Fear, select = c(1:6)))        # summing variables to get Total Score for this factor
Fear$FearTotal <- (Fear$FearTotal/6)
#Factor3 [complexity/burden of making data accessible] (DS13_01, DS13_02, DS13_03, DS13_04, PR07_03)

Complexity<- as.data.frame(cbind(OSQ_daten$DS13_01,
                                 OSQ_daten$DS13_02,
                                 OSQ_daten$DS13_03,            
                                 OSQ_daten$DS13_04,
                                 OSQ_daten$PR07_03))
...
Complexity$ComplexityTotal <- rowSums(subset(Complexity, select = c(1:5))) 
Complexity$ComplexityTotal <- (Complexity$ComplexityTotal/5)
...
Follow_up$TrainingTotal<- Training$TrainingTotal                                  
...
t.test(Follow_up$TrainingTotal ~ researchexp)
...
t.test(Follow_up$TrainingTotal ~ EU)
...
t.test(Follow_up$DS09 ~ researchexp)
t.test(Follow_up$DS10 ~ researchexp)
...
Graph<- na.omit(Follow_up)

#plot Datagovernance_Uni
Graph$University<- as.factor(Graph$University)

plot<-ggplot(Graph, aes(x= University, y=ControlTotal, fill=University))+ 
  geom_violin()+
  geom_boxplot(width=0.1, color="black", alpha=0.2)+
  theme_classic()+
  labs(x= "Primary affiliation", y= "Need for data governance")+
  scale_x_discrete(labels=c("University Hospital/Medical faculty", "Other university faculty"))+
  scale_y_discrete(limits=c("1", "2", "3", "4", "5", "6", "7"))
plot

ggsave(file="Datagovernance_Uni_plot_white.svg", plot=plot)
ggsave(file="Datagovernance_Uni_plot_black.svg", plot=plot)

#plot DSO9_EU
Graph$EU<- as.factor(Graph$EU)
plot2<-ggplot(Graph, aes(x= EU, y=DS09, fill=EU))+ 
  geom_violin()+
  geom_boxplot(width=0.1, color="black", alpha=0.2)+
  theme_classic()+
  labs(x= "EU residency", y= "")+
  ggtitle('"How likely are you to share primary research data \n for your next neuroimaging paper in an online repository"')+
  theme(plot.title = element_text(size=10, hjust=0.5))+
  scale_x_discrete(labels=c("EU residents", "Non-EU residents"))+
  scale_y_discrete(limits=c("1", "2", "3", "4", "5"))
plot2

ggsave(file="DS09_EU_plot.svg", plot=plot2)

#plot DS09_researchexperience 
Graph$researchexp<- as.factor(Graph$researchexp)
plot3<-ggplot(Graph, aes(x= researchexp, y=DS09, fill=researchexp))+ 
  geom_violin()+
  geom_boxplot(width=0.1, color="black", alpha=0.2)+
  labs(x= "Research Experience", y= "")+
  ggtitle('"How likely are you to share primary research data \n for your next neuroimaging paper in an online repository"')+
  theme(plot.title = element_text(size=10, hjust=0.5))+
  scale_x_discrete(labels=c("less than 16 years", "more than 16 years"))+
  scale_y_discrete(limits=c("1", "2", "3", "4", "5"))
plot3

ggsave(file="DS09_researchexp_plot.svg", plot=plot3)

#plot DS10_researchexp
plot4<-ggplot(Graph, aes(x= researchexp, y=DS10, fill=researchexp))+ 
  geom_violin()+
  geom_boxplot(width=0.1, color="black", alpha=0.2)+
  theme_classic()+
  labs(x= "Research Experience", y= "")+
  ggtitle('"I am not allowed to share my primary neuroimaging data"')+
  theme(plot.title = element_text(size=10, hjust=0.5))+
  scale_x_discrete(labels=c("less than 16 years", "more than 16 years"))+
  scale_y_discrete(limits=c("1", "2", "3", "4", "5"))
plot4

ggsave(file="DS10_researchexp_plot.svg", plot=plot4)

#plot DS10_EU 
plot5<-ggplot(Graph, aes(x= EU, y=DS10, fill=EU))+ 
  geom_violin()+
  geom_boxplot(width=0.1, color="black", alpha=0.2)+
  theme_classic()+
  labs(x= "EU residency", y= "")+
...
ggsave(file="DS10_EU_plot.svg", plot=plot5)

>>>>>>> ea47c2dfee7ecc8e7ea462268a04629ecf39ae5a:code for follow up analyses/Datagovernance_University_plot.R
